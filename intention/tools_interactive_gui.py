...
class PolicyOutput(QtWidgets.QWidget):
    ...
    def refresh(self):
        ...
        if 'output_low' in info:
            policy_output = info['output_low']
        elif 'output_mid' in info:
            policy_output = info['output_mid']
        else:
...
class DraggableCubePolygon(DraggablePolygon):
    def __init__(self, canvas, object_id):
        self.env = canvas.main_window.env
        object_xy = ((self.env.object_width / 2) * np.array([[-1, 1], [1, 1], [1, -1], [-1, -1]])).tolist()
        polygon = Polygon(object_xy, True, color=VectorEnv.OBJECT_COLOR)
        super().__init__(canvas, polygon)
        self.env_mutex = self.canvas.main_window.env_mutex
    ...
    def on_new_pose(self):
        if self.env_mutex.tryLock():
            self.env.reset_object_pose(self.object_id, self.position[0], self.position[1], self.heading)
            self.env.step_simulation()
            self.canvas.main_window.refresh()
            self.env_mutex.unlock()
...
class VectorEnvCanvas(FigureCanvas):
    def __init__(self, main_window):
        ...
        for object_id in self.env.object_ids:
            self.object_polygons.append(DraggableCubePolygon(self, object_id))

    def refresh(self):
        for robot, polygon in zip(self.env.robots, self.robot_polygons):
            polygon.set_pose(robot.get_position()[:2], robot.get_heading())
        for object_id, polygon in zip(self.env.object_ids, self.object_polygons):
            object_position, object_orientation = self.env.get_object_pose(object_id)
            hidden = (object_position[2] < 0.9 * VectorEnv.REMOVED_BODY_Z)
            polygon.set_pose(object_position[:2], orientation_to_heading(object_orientation), hidden=hidden)
        self.draw()
...
def main(args):
    ...
    if args.config_path is not None:
        ...
        robot_config = [{'blowing_robot': 1}]
        #robot_config = [{'moving_blowing_robot': 1}]
        #robot_config = [{'side_blowing_robot': 1}]
